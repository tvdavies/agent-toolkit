# Pi sessions source connector

Standalone source connector for importing local Pi Coding Agent session JSONL files into Brain.
...
```sh
brain sources add-command pi-local \
  --kind pi-session \
  --command 'bun /path/to/ai-assistant/packages/connectors/pi-sessions/src/index.ts'
```
...
brain sources add-command pi-local \
  --kind pi-session \
  --command 'bun /path/to/ai-assistant/packages/connectors/pi-sessions/src/index.ts --limit 200 --root ~/.pi/agent/sessions'
```

## Sync

```sh
brain sources sync pi-local --mode archive
```
