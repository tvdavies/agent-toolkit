...
- Native pi-subagents launches one model and reports provider/quota failures without rotating models, even before tool activity. A failed run keeps its partial work: report the run and worktree state, inspect and preserve the diff, and ask the user before a new, explicit continuation task on another approved model. Do not replay completed changes and do not resume a retained child expecting a different model.
