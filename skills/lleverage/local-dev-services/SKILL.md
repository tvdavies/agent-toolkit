...
cd "$LLEVERAGE_REPO" && docker compose up -d
```

When using a shell tool, set its working directory to the resolved `LLEVERAGE_REPO` rather than the current worktree.
