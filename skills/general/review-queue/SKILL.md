...
Shows how many PRs each configured team member has reviewed in a given period. Set `REVIEW_TEAM_MEMBERS` to a comma-separated list; when omitted, the script reports the authenticated GitHub user.
...
REVIEW_TEAM_MEMBERS=alice,bob ~/.agents/skills/review-queue/scripts/review-stats.sh --repo example/widgets
```
