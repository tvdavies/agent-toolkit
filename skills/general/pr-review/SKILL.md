...
Extract the build ID and any project/region identifiers from the check URL. Use repository documentation, authenticated `gcloud` configuration, or explicit environment variables for the project, region, and account; never fall back to organisation-specific values. For example:
```bash
gcloud builds describe BUILD_ID \
  ${CLOUD_BUILD_REGION:+--region="$CLOUD_BUILD_REGION"} \
  ${CLOUD_BUILD_PROJECT:+--project="$CLOUD_BUILD_PROJECT"} \
  ${CLOUD_BUILD_ACCOUNT:+--account="$CLOUD_BUILD_ACCOUNT"} \
  --format='json(status,steps.id,steps.status)'
```

If any step failed, get the raw logs with the same resolved flags and extract the relevant failure output:
```bash
gcloud builds log BUILD_ID \
  ${CLOUD_BUILD_REGION:+--region="$CLOUD_BUILD_REGION"} \
  ${CLOUD_BUILD_PROJECT:+--project="$CLOUD_BUILD_PROJECT"} \
  ${CLOUD_BUILD_ACCOUNT:+--account="$CLOUD_BUILD_ACCOUNT"} 2>&1 \
  | grep -A 50 "Step #N.*FAILED\|error TS\|ERR!\|FAIL " | head -60
```
...
2. Retrieve the previous review via `scripts/fetch-conversation.sh --pr PR_NUMBER`, which returns a bounded newest-first window of review bodies (approval and request-changes rounds), issue comments (changes-suggested rounds), and line-comment threads, and reports truncation. Take the most recent item whose H2 heading marks a verdict: "Approved", "Approved with Suggestions", "Changes Suggested", or "Changes Requested". The inline findings attached to that round appear in the line-comment threads section of the same output; match them to the selected round by the review id shown on both the review entry and each thread. If the retrieval fails (non-zero exit), stop and report it — do not treat a failed fetch as "no previous review". Do not paginate GitHub conversation history unboundedly by hand.
