...
4. Assign the issue to yourself and move it to **In Progress**:
   ...
   If `issues start` is unavailable, use `issues assign TEAM-123 --assignee me` and `issues update TEAM-123 -s "In Progress"` with the standard JSON/no-pager flags. Verify both fields with a fresh `issues get` call.
