...
Keep the description concise but complete. Unless the repository or ticket requires a different template, use the useful sections that apply:

- `## Summary` — the outcome in short bullets
- `## Context` — why the change is needed, with relevant ticket or incident links
- `## Changes` — important implementation details and decisions
- `## Validation` — exact commands and honest results
- `## Risk / Rollback` — operational risk and a practical rollback path

Omit empty sections rather than leaving placeholders. Never invent validation results; if a check was not run, write `Not run` and the reason. Draft PRs should still have a production-quality body.
...
Resolve this skill's installed directory and run `scripts/write-pr-body.sh [OUTPUT]` to create the default template (normally `/tmp/pr-body.md`) before editing and passing it to `gh ... --body-file`.
